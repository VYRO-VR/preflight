export const en = {
  ...
  // Firmware picker (board + options → exact file)
  'fw.pick.board': 'Your tracker board',
  'fw.pick.board.receiver': 'Your receiver board',
  'fw.pick.board.placeholder': 'Select your board…',
  'fw.pick.detected': 'Detected {board} — change it below if that’s not right.',
  'fw.pick.mag': 'Magnetometer',
  'fw.pick.mag.explain':
    'Uses the onboard compass for more stable yaw (less drift over time). Leave off if you’re near metal or magnets, or if you were told to.',
  'fw.pick.sleep': 'Deep sleep',
  'fw.pick.sleep.explain':
    'Powers the tracker down after it sits still for a while to save battery; a button press wakes it. Turn off only if your tracker won’t wake up reliably.',
  'fw.pick.on': 'On',
  'fw.pick.off': 'Off',
  'fw.pick.advanced': 'Advanced options',
  'fw.pick.protocol': 'Radio protocol',
  'fw.pick.protocol.hint': 'Trackers and the receiver must use the same protocol.',
  'fw.pick.bus': 'IMU connection',
  'fw.pick.clock': 'External clock',
  'fw.pick.sw0': 'SW0 wiring',
  'fw.pick.fork': 'Build variant',
  'fw.pick.file': 'Firmware file',
  'fw.pick.closest':
    'That exact combination isn’t built — showing the closest available file. Adjust the options if it’s not what you want.',
  'fw.pick.none': 'No firmware file matches that selection.',
  'fw.pick.download': 'Download {name}',
  'fw.pick.downloaded': 'Saved to {path}',
  'fw.opt.protocol.standard': 'Standard (ESB)',
  'fw.opt.protocol.tdma': 'TDMA',
  'fw.opt.bus.i2c': 'I²C',
  'fw.opt.bus.spi': 'SPI',
  'fw.opt.bus.smspi': 'smSPI',
  'fw.opt.bus.none': 'Default',
  'fw.opt.clock.default': 'Default',
  'fw.opt.clock.clk': 'External clock',
  'fw.opt.clock.noclk': 'No external clock',
  'fw.opt.fork.standard': 'Standard',
  'fw.opt.fork.jitingcat': 'JitingCat',
