...
Current tracker firmware streams a sens-cal report (phase, result code, computed scale, integrated angle) over the radio, which the receiver echoes on its console as a `Sens cal tracker …` line (`RECEIVER_CONSOLE.sensCalReportRegex`, enums in `SENS_CAL_PHASE`/`SENS_CAL_RESULT`). When those lines are fresh they drive the phase machine and the verdict; when they are absent or stale (`SENS_CAL.reportStaleMs`) the machine falls back to inferring the phase from the receiver's ack, the orientation feed, and its own copy of the firmware timeouts. The verification spin measures the residual either way. Slot ↔ tracker identity has no protocol link: `@shared/receiver-slots.ts` matches the address inside the SlimeVR tracker name as a hint, and the flow always confirms with the user before sending.
